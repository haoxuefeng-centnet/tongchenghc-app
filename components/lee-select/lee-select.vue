<template>
  <view class="select-page">
        ...
        <view class="sort-wrap" v-for="(sort,index1) in listData" :key="index1" :id="'view'+ index" :style="{backgroundColor:getListAttrListBackgroundColor}">
          ...
          <view class="list">
            <text v-for="(city, index2) in sort.list" :key="index2" :style="{height:getListAttrItemHeight, lineHeight:getListAttrItemHeight,  fontSize:getListAttrItemFontSize,borderBottom:getListAttrItemBorderBottom,margin:getListAttrItemMargin,color:getListAttrItemColor,background:getListAttrItemBackgroundColor}"
              @click="chooseItem(city.name)">{{city.name}}</text>
          </view>
        </view>
      </view>
    </scroll-view>
    ...
    <view class="letter-nav" v-if="getNavAttrEnable" :style="{padding:getNavAttrPadding,borderRadius:getNavAttrBorderRadius}">
      <text :class="['item',index === activeIndex ? 'active': '']" v-for="(item,index) in getNavData" :key="index"
        @click="scrollSelect(index)" :style="{backgroundColor:index === activeIndex ? getNavAttrActiveColor : '',color:index === activeIndex ? getNavAttrActiveColors : '', fontSize:getNavAttrFontSize,padding:getNavAttrItemPadding}">{{item}}</text>
    </view>
  </view>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      getNavAttrColor() {
        return this.navAttr.color || ''
      },
      getNavAttrActiveColor() {
        return this.navAttr.activeColor || 'rgba(0, 0, 0, 0.4)'
      },
      getNavAttrActiveColors() {
        return this.navAttr.activeColors || '#fff'
      },
      getNavAttrFontSize() {
